		...
		<div className="flex items-center gap-2 px-4 py-2 border-b border-foreground/10 bg-editor-panel">
			<div className="flex items-center gap-1">
				<Button
					...
					variant="ghost"
					size="icon"
					className="h-7 w-7 text-muted-foreground hover:text-[#6D4FD1] hover:bg-[#6D4FD1]/10 transition-all"
					title={addZoomLabel}
			...
			<div className="flex items-center gap-2">
					...
					>
						{ASPECT_RATIOS.map((ratio) => (
							...
							>
								...
								{aspectRatio === ratio && (
									<Check className="w-3 h-3 text-[#6D4FD1]" />
								)}
							</DropdownMenuItem>
						))}
						...
						<div className="px-2 py-1.5 flex items-center gap-2 text-muted-foreground">
							...
							<input
								...
								className="w-12 h-7 rounded border border-foreground/10 bg-foreground/5 px-1.5 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-[#6D4FD1]"
							...
							{isCustomAspectRatio(aspectRatio) && (
								<Check className="w-3 h-3 text-[#6D4FD1] ml-auto" />
							)}
						</div>
					</DropdownMenuContent>
				</DropdownMenu>
				...
				>
					...
					{isCropped ? <span className="h-1.5 w-1.5 rounded-full bg-[#6D4FD1]" /> : null}
				</Button>
			</div>
			<div className="flex-1" />
			<div className="flex items-center gap-4 text-[10px] text-muted-foreground/70 font-medium">
				<span className="flex items-center gap-1.5">
					<kbd className="px-1.5 py-0.5 bg-foreground/5 border border-foreground/10 rounded text-[#6D4FD1] font-sans">
						Side Scroll
					</kbd>
					<span>Pan</span>
				</span>
