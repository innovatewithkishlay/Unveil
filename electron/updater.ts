...
function applyExperimentalUpdatesPreference() {
	...
	// Changing channels enables downgrades inside electron-updater. Unveil never
	...
	return {
		...
		detail:
			normalizedProgress >= 100
				? "Finishing the update download. Unveil will restart as soon as the installer is ready."
				: remainingMb !== null
					? `${remainingMb.toFixed(1)} MB left before Unveil restarts.`
					: "Downloading the update now. Unveil will restart when it finishes.",
		delayMs: UPDATE_REMINDER_DELAY_MS,
		isExperimental,
...
export async function downloadAvailableUpdate(
	...
	setUpdateStatusSummary({
		status: "downloading",
		availableVersion,
		detail: `Downloading Unveil ${availableVersion}`,
	});
...
async function showAvailableUpdateDialog(
	...
	const result = await showMessageBox(getMainWindow, {
		...
		message: `Unveil ${version} is available${isExperimental ? " on the experimental channel" : ""}.`,
		detail: isPreview
...
async function showDownloadedUpdateDialog(
	...
	const result = await showMessageBox(getMainWindow, {
		...
		title: "Update Ready",
		message: isPreview
			? `Unveil ${version} is ready to install.`
			: `Unveil ${version} has been downloaded.`,
		detail: isPreview
...
async function showUpdateErrorDialog(
	...
	await showMessageBox(getMainWindow, {
		type: "error",
		title: "Update Failed",
		message: `Unveil ${version} could not be downloaded.`,
		detail: String(error),
		buttons: ["OK"],
		defaultId: 0,
		noLink: true,
...
export function setupAutoUpdates(
	...
	autoUpdater.on("update-available", (info) => {
		...
		setUpdateStatusSummary({
			status: "available",
			availableVersion: info.version,
			detail: `Unveil ${info.version} is available.`,
		});
	...
	autoUpdater.on("update-not-available", () => {
		...
		setUpdateStatusSummary({
			status: "up-to-date",
			availableVersion: null,
			detail: `Unveil ${app.getVersion()} is up to date.`,
		});
		clearVisibleUpdateToast(sendToRenderer);
		manualCheckRequested = false;
	});
	...
	autoUpdater.on("update-downloaded", (info) => {
		...
		setUpdateStatusSummary({
			status: "ready",
			availableVersion: info.version,
			detail: `Unveil ${info.version} is ready to install.`,
		});
		clearDeferredReminderTimer();
