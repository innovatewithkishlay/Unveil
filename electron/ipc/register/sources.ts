		...
		if (process.platform !== "darwin" || !includeWindows) {
			const windowSources = electronSources
				...
				.filter((source) => {
					...
					if (ALLOW_RECORDLY_WINDOW_CAPTURE && normalizedName.includes("unveil")) {
						return true;
					}
		...
		try {
			...
			const mergedWindowSources = nativeWindowSources
				.filter((source) => {
					...
					if (
						ALLOW_RECORDLY_WINDOW_CAPTURE &&
						(normalizedAppName === "unveil" ||
							normalizedWindowName?.includes("unveil"))
					) {
						return true;
					}
...
.border-wrap{
  ...
  background:conic-gradient(from var(--angle,0deg),
    transparent 0%,
    transparent 60%,
    rgba(109, 79, 209,.15) 70%,
    rgba(109, 79, 209,.9) 80%,
    rgba(190, 165, 245,1) 85%,
    rgba(109, 79, 209,.9) 90%,
    rgba(109, 79, 209,.15) 95%,
    transparent 100%
  );
...
.glow-wrap{
  ...
  background:conic-gradient(from var(--angle,0deg),
    transparent 0%,
    transparent 65%,
    rgba(109, 79, 209,.3) 78%,
    rgba(190, 165, 245,.5) 85%,
    rgba(109, 79, 209,.3) 92%,
    transparent 100%
  );
  -webkit-mask:linear-gradient(#fff 0 0) content-box,linear-gradient(#fff 0 0);
  -webkit-mask-composite:xor;
