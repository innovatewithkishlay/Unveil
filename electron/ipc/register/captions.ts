	...
	ipcMain.handle("open-video-file-picker", async (_, options?: OpenVideoFilePickerOptions) => {
		try {
			...
			const result = await dialog.showOpenDialog({
				title: includeProjects ? "Import Media or Unveil Project" : "Select Video File",
				defaultPath: recordingsDir,
				filters: [
					...(includeProjects
						? [
								{
									name: "Media or Unveil Projects",
									extensions: [
					...
					...(includeProjects
						? [{ name: "Unveil Projects", extensions: PROJECT_FILE_EXTENSIONS }]
						: []),
