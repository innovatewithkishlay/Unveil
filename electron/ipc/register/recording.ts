		...
		async (_, source: SelectedSource, options?: NativeMacRecordingOptions) => {
				...
				try {
					...
					if (options?.capturesSystemAudio) {
						...
						tempSystemAudioPath = path.join(
							app.getPath("temp"),
							`unveil-native-${timestamp}.system.wav`,
						);
					...
					if (options?.capturesMicrophone && !browserMicFallbackRequested) {
						...
						tempMicPath = path.join(
							app.getPath("temp"),
							`unveil-native-${timestamp}.mic.wav`,
						);
			...
			try {
				...
				if (
					...
					appName &&
					(appName === ownAppName || appName === "unveil")
				) {
					return {
						success: false,
						message:
							"Cannot record Unveil windows. Please select another app window.",
					};
				}
				...
				if (
					...
					const { response } = await dialog.showMessageBox({
						...
						message:
							"Unveil needs screen recording permission to capture your screen.",
						detail: "Please open System Settings > Privacy & Security > Screen Recording, make sure Unveil is toggled ON, then try recording again.",
				...
				if (errorStr.includes("MICROPHONE_PERMISSION_DENIED")) {
					const { response } = await dialog.showMessageBox({
						...
						message: "Unveil needs microphone permission to record audio.",
						detail: "Please open System Settings > Privacy & Security > Microphone, make sure Unveil is toggled ON, then try recording again.",
						buttons: ["Open System Settings", "Cancel"],
						defaultId: 0,
						cancelId: 1,
					});
