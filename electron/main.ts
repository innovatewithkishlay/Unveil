...
function closeEditorWindowToHud(window: BrowserWindow | null) {
	...
	// the editor, keeping Unveil in its ready-to-record state on the taskbar.
	window.hide();
...
function getPlatformAppIconFilename(size: 32 | 128 | 512) {
	const baseName = process.platform === "darwin" ? "unveilmac" : "unveil";
...
function shouldUseTray() {
	// macOS and Windows expose Unveil through their Dock/taskbar. Keep the
...
function updateTrayMenu(recording: boolean = false) {
	...
	const trayToolTip = recording ? `Recording: ${selectedSourceName}` : "Unveil";
...
app.whenReady().then(async () => {
	if (process.platform === "win32") {
		app.setAppUserModelId("com.unveil.app");
	}
	...
	// Unveil does not use WebHID, Web Serial, or WebUSB. Do not grant devices by default.
