...
const WINDOW_ICON_FILENAME =
	process.platform === "darwin" ? "unveilmac-512.png" : "unveil-512.png";
...
export function createHudOverlayWindow(): BrowserWindow {
	...
	const win = new BrowserWindow({
		...
		resizable: false,
		alwaysOnTop: true,
		// The HUD is Unveil's persistent top-level window, so it owns the
	...
	const showHudWindow = () => {
		...
		if (process.platform === "win32") {
			...
			// always-on-top HUD must not steal focus when Unveil starts.
			win.showInactive();
		} else {
			win.show();
		}
...
export function createUpdateToastWindow(): BrowserWindow {
	...
	win.setVisibleOnAllWorkspaces(true, {
		visibleOnFullScreen: true,
		// Keep Unveil a foreground application so macOS does not temporarily
